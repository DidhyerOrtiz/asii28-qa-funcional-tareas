...
- `ContextoEjecucion` identifica versión, ambiente, tenant y rol.
...
- El resultado conserva el identificador del caso y la versión evaluada.
...
| `CasoManual` | Ejecutar pasos manuales ya validados y devolver el resultado uniforme. | Aceptar datos incompletos como si fueran ejecutables. |
| `CasoReprueba` | Repetir un caso relacionado con un defecto y conservar esa trazabilidad. | Fortalecer las precondiciones de `ejecutar()`. |
...
| `ContextoEjecucion` | Transportar versión, ambiente, tenant y rol comunes a la ejecución. | Decidir el resultado del caso. |
| `ResultadoEjecucion` | Representar estado, resultado observado, evidencia y causa de bloqueo. | Ejecutar pasos o crear defectos por sí mismo. |
| `GestorCampana` | Recorrer casos ejecutables y coordinar el registro de resultados. | Preguntar si un objeto es `CasoManual`, `CasoReprueba` o `CasoBorrador`. |
...
CasoRegresionEjecutable`. El gestor conoce la promesa observable, no los detalles de
cada variante. `CasoBorrador` queda en otro flujo y solo puede entrar a una campaña
después de que `ValidadorCaso` produzca un objeto válido.
